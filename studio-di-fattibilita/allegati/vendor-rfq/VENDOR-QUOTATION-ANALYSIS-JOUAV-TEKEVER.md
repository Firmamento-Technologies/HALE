...
## Stima preliminare CapEx + TCO 5 anni per Percorso 6A pilota Pentema
...
> **Riferimento Studio:** Cap. 6 §6.3.1 TS-PLATFORM-6A + Cap. 8 §8.3.1, §8.4.1
> **Skill applicate:** `vtol-uas-specialist` + `trade-study-analysis` + `epistemic-rigor` + `competitor-intelligence`
> **Versione:** 1.0 — bozza M+3
> **Data:** [DD/MM/YYYY]
> **Owner analisi:** Procurement Manager + Lead Systems Engineer Firmamento
...
>
> Le stime servono per:
> - Validare il **range CapEx Y1** dello Studio (€700-1200k Briefing vs €975k-1.96M Cap. 8 §8.3.1)
> - Identificare il **gap potenziale** tra Plan A (JOUAV) e Plan B (Tekever)
> - Supportare la **strategia di funding** Y1 (mix Coopfond + FESR + equity)
> - Decidere lo **stockpile spare strategico** in funzione del rischio supply chain
>
...
|---|---|---|---|
| **Plan A** | **JOUAV CW-30E** (CN) | **Baseline preferred** | Best payload (8 kg) + costo iniziale inferiore (~30-40% meno di Tekever) + 200 km C2 range |
| **Plan B** | **Tekever AR3** (PT) | **EU sovereign fallback ready** | NATO-certified, reference EU strong (UK Home Office, EMSA, RAF), autonomia 16h, ma costo +30-40% e payload ridotto (4 kg) |

**Pronto switch automatico Plan A → Plan B** se:
- Sanzioni / restrizioni export US-CN bloccano JOUAV entro M+6 (probabilità: medium, sezione 5.3 sotto)
- Golden Power notifica respinta dalla Presidenza Consiglio Ministri (probabilità: low-medium)
- ENAC notifica restrizioni operative su vendor CN per BVLOS in IT (probabilità: low)
- Decisione strategica di Firmamento per allineamento EU sovereign narrative (probabilità: medium, dipende da signal political)
...
| **OpEx + manutenzione + spare 5 anni** | 280-450 | 350-550 | +70-100 |
| **TCO 5 anni totale (no IVA)** | **860-1.270** | **1.200-1.800** | **+340-530** |
| **Lead time consegna full system** | 6-9 mesi | 8-14 mesi | +2-5 mesi |

> **CONFIDENCE: low-medium**. Range ampio per riflettere incertezza intrinseca su quotation non ricevute. Le quotation reali possono divergere ± 50% da queste stime.

### 1.3 Falsifying observations chiave (epistemic-rigor Regola 5)

- **FO-1**: Se quotation JOUAV reale > 50% sopra range stima (es. > €1.2M CapEx), **re-baseline** scope MVP: eliminare payload telecom Y1, ridurre GS mobile, considerare Tekever.
- **FO-2**: Se quotation JOUAV reale < 50% sotto range stima (es. < €290k CapEx), **scope expansion**: aggiungere LiDAR + multispettrale Y1.
- **FO-3**: Se Tekever quotation > €1.5M CapEx, switch fallback secondario su **Quantum Trinity F90+** (DE) — capability ridotta ma EU sovereign + costo dimezzato.
- **FO-4**: Se lead time JOUAV > 12 mesi (rilevato in RFQ response), attivazione immediata path Tekever indipendentemente da costo (vincolo timing Y1 MVP critico).
...
| **Status export US** | Non in Entity List BIS (verificato M+3) ma supply chain componenti soggetta a US Section 301 tariffs (25%) + (temporaneo) Section 122 surcharge 10% se passaggio US |
...
**Caveat:** ogni valore vendor è **datasheet**, non validato da operatori EU indipendenti. Performance reale a Pentema (1100-1300 m AMSL, vento canalizzato, inverno -5°C) probabilmente **-20/40% rispetto datasheet** (vedi Cap. 6 §6.2.1).

### 2.3 Stima quotation JOUAV CW-30E (CapEx + TCO 5 anni)

**Metodologia:** triangolazione tra:
1. Pricing tipico VTOL fixed-wing 30-40 kg MTOW mercato CN (range $50-150k USD piattaforma base)
2. Markup reseller EU + customs + shipping (tipico +30-50%)
3. Aggiunte payload + GS + training a prezzi mercato standard
4. Best practice aerospace UAV procurement (overhead, contingency)
...
| **Payload IR LWIR (WIRIS Pro o FLIR Vue Pro R)** | 20-50 | M | WIRIS Pro ~€20-30k, FLIR ~€15-25k |
...
- Produzione + FAT: 3-5 mesi (vendor CN tipico)
- Shipping CN → IT: 1-2 mesi (sea freight); 2-3 settimane (air, +costi)
- Dogana Italia: 2-4 settimane
- Training + SAT + commissioning: 1-2 mesi
- **TOTALE end-to-end stimato: 6-9 mesi** (best case); **9-14 mesi** (worst case con delays customs/sanctions)
...
| **Dimensione** | Mid-cap aerospace EU, ~600+ dipendenti, fatturato stimato €100M+ (2024-2025) |
...
| **Track record settore civile + dual-use** | Surveillance marittima, EMSA SAR (Search And Rescue), border surveillance, electronic warfare (StormShroud), Ucraina deployment (battle-proven) |
...
| Velocità crociera | ~70-90 km/h | `[vendor]` | medium |
| Velocità max | ~110 km/h | `[vendor]` | medium |
| Quota max operativa | ~3500-4000 m AMSL | `[vendor]` | medium |
| Range operativo | 100 km surveillance radius | `[vendor + Wikipedia + UST]` | high |
| Operating temperature | -20°C / +50°C | `[vendor]` | medium |
| Vento sostenuto max | ~13-15 m/s | `[vendor]` | medium |
...
### 3.3 Stima quotation Tekever AR3 (CapEx + TCO 5 anni)

**Metodologia:** triangolazione tra:
1. Pricing tipico UAV fixed-wing 25 kg MTOW EU-sovereign aerospace mid-tier ($150-400k USD piattaforma base)
2. Benchmark EMSA contract €30M / 4 anni / multi-vendor REACT consortium = stima €2-3M per drone-anno operativo all-inclusive (con servizi)
3. UK Home Office £1B / 6 anni framework = stima impossibile per breakdown (incluso operations as a service)
4. Premium EU sovereign + NATO certification + reference customer EU strong
...
- Produzione + FAT: 4-7 mesi (vendor EU mid-tier, backlog NATO orders)
- Shipping PT → IT: 1-2 settimane (intra-EU, no customs)
- Training + SAT + commissioning: 2-3 mesi (più approfondito NATO-grade)
- **TOTALE end-to-end stimato: 8-12 mesi** (best case); **12-16 mesi** (worst case con backlog ordini militari)

**Confidence lead time: medium** (backlog NATO ordini Ucraina + RAF + UK Home Office potrebbe estendere lead time per ordini civili commerciali low-priority).

---

## 4. Stima Quotation Strutturata Confronto

### 4.1 Tabella comparativa CapEx + TCO
...
**Lettura:**
- Plan A JOUAV è **al di sotto del range** Studio: spazio di manovra OK
- Plan B Tekever è **al margine superiore del range**, ma ancora **dentro il range realistico** Cap. 8 (no contingency overrun)
- Se Tekever quotation supera €1.5M CapEx Y1, **scope expansion non possibile** entro budget; necessario funding mix più aggressivo (+ €300-500k FESR/PNRR) o de-scoping (no payload telecom Y1)
...
- **EU non ha tariffe specifiche** su drone CN (al M+3), MA Italia ha possibilità di applicare Golden Power

**Impatto su Firmamento:**
- Acquisto JOUAV direttamente da reseller EU **non soggetto a tariffe US** (non passa via US)
- Componenti US-origin in JOUAV (chip, sensori): se restrittivi US Entity List, possibile re-export limitations
- **Probabilità di blocco totale CW-30E export to Italia: low (10-15%)** entro 12 mesi

#### 5.1.2 Restrizioni export CN su drone parts (2025-2026)

**Trend osservato:**
- Cina ha imposto **export controls su drone parts** dal dicembre 2024 (lithium batteries, motors, flight controllers per US/EU/Ukraine)
- 28 entità US aggiunte a Chinese export control list (gennaio 2025)
- Dicembre 2024: China optimizes export control measures for drones, ban export intended for military purposes

**Impatto su Firmamento:**
- Acquisto JOUAV per uso **civile commerciale documentato** non dovrebbe essere bloccato (vendor cinese rilascia end-user statement)
- MA processo customs CN può **rallentare lead time** di 2-4 mesi
- Ricambi (motori, batterie, FCS) possono essere intermittentemente bloccati con preavviso breve
...
**Status M+3:**
- FCC ha bloccato import di nuovi drone DJI e foreign companies in tarda 2025
- Drone già importati possono continuare a essere venduti (es. DJI Mini 5 Pro, Mavic 4 Pro)
- Commerce Department ha **abbandonato proposta ban completo** drone CN gennaio 2026 dopo revisione interna

**Impatto su Firmamento:**
- Ban non riguarda direttamente JOUAV CW-30E (categoria diversa da DJI consumer)
- MA crea **precedente politico** che può estendersi: Italia potrebbe seguire path US (probabilità bassa al M+3, ma observable trigger)
- Reputational risk: in scenario di escalation, JOUAV come "Chinese aerospace" può attrarre scrutinio mediatico

**Probabilità di replicazione ban in EU: low (15-20%)** entro 36 mesi

#### 5.1.4 Risk Golden Power notifica

**Inquadramento normativo:** D.L. 21/2012 + modif. D.L. 23/2020 (estensione settore aerospace + dual-use) + D.L. 50/2022.

**Applicabilità al caso JOUAV:**
- Acquisizione **non costituisce M&A** (è procurement asset, non acquisizione società)
- MA D.L. 21/2012 art. 2 c. 1-ter copre "acquisizioni di tecnologie e attivi strategici" da extra-UE
- Aerospace è settore Golden Power "ad alta strategicità"
- Vendor CN è **extra-UE non amico** (in lista di paesi sensibili)

**Probabilità di triggering notifica obbligatoria: medium-high (60-70%)**

**Probabilità di esito negativo (veto Golden Power): low (10-20%)**
- Razionale: piattaforma è **commerciale civile**, payload sono **non sensibili**, use case è **Aree Interne italiane**, end user è **PA italiana**
- Mitigazione: documentazione end-use, audit supply chain, certificazione no military-grade payload

**Tempi:** notifica → istruttoria → risposta tipicamente **45 gg lavorativi** (può estendersi a 90 gg con richiesta integrativa)

**Costi:** legale specialistica Golden Power = €15-30k + tempi gestionali interni Firmamento (2-3 mesi PM full-time equivalent)

**RACCOMANDAZIONE OPERATIVA:**
- Inserire **Golden Power notifica** come tappa OBBLIGATORIA del cronoprogramma vendor JOUAV (M+8-10)
- Pre-engagement informale con **DPSCS** (Dipartimento per le Politiche Strategiche e Cooperazione Internazionale) Presidenza Consiglio Ministri per **pre-screening informale** prima di formal notifica
- Documentazione preparata in anticipo: end-use statement + supply chain audit + cybersecurity attestation

#### 5.1.5 Vendor concentration risk

**JOUAV è il vendor:** single point of failure su un singolo produttore CN.

**Mitigazioni Firmamento:**
1. Stock spare critici 12-18 mesi (vs 3 anni standard): protezione gap supply
2. Contratto vendor con **clausole continuità** e penali per disruption
3. **Plan B Tekever pre-validato e contract-ready** (vedi sez. 6 sotto)
4. Path migrazione documentato: come switchare da JOUAV a Tekever in 6-9 mesi

### 5.2 Risk Tekever AR3 (Plan B) — analisi dettagliata

#### 5.2.1 Risk geopolitico Tekever

**Vantaggi EU sovereign:**
- Vendor PT, intra-UE → **no customs, no export licensing, no Golden Power triggering**
- NATO eligible vendor → allineamento con sovereign IT/EU narrative
- Reference customer EU strong → bassa reputational risk
- Supply chain Tekever EU-prevalente → ridotta exposure US/CN tensioni
...
**Reference customer score:** Tekever è **5+/5** su criterio "reference EU operatori" (Cap. 6 §6.3.1 criterio "Reference customers EU").
JOUAV è **1-2/5** (reference EU **non documentati** pubblicamente per CW-30E specifico, alcuni operatori privati anecdotali).

---

## 6. Matrice Decisionale Pesata (Trade Study)

Riferimento: skill `trade-study-analysis` + matrice Cap. 6 §6.3.1.
...
**Razionale dello shift:**
1. **Capability JOUAV ridimensionata** (5 vs 7 su EASA/ENAC compliance — confermata mancanza evidenze EASA esplicite per CW-30E specifico)
2. **Reference customer JOUAV** ridimensionata (3 vs 8 — confermata mancanza reference EU pubblicamente documentate)
3. **Risk geopolitico JOUAV** confermato critico (4 vs 5)
4. **Costo JOUAV** rimane vantaggio (8 vs 5)
...
> 4. Budget Firmamento Y1 esteso a €1.0-1.5M (mix funding + Series A seed)
> 5. Signal politico EU sovereign chiaro (es. IRIS² Italia node + EDF call HAPS preferences EU vendor)
>
> **Mantieni JOUAV come baseline** se:
> 1. Quotation JOUAV < €500k CapEx → vantaggio costo permanente
> 2. Budget Y1 vincolato a €600-900k (Briefing originale)
> 3. Speed-to-market è critico (lead time < 6 mesi vincolo hard)
> 4. Cooperative Legacoop accept "vendor CN" senza obiezioni reputational
>
> **DECISIONE NEUTRA (M+3 attuale, in attesa quotation reali):**
> Procedere con **doppia RFQ formale parallela** JOUAV + Tekever, con valutazione finale a M+6 sulla base di quotation effettive. Mantenere apertura su Plan B Quantum F90+ (DE) come Plan C in caso entrambi falliscano.
...
**Le stime in questo documento sono basate su:**
1. Datasheet vendor pubblici (JOUAV.com, Tekever.com) — confidence medium (vendor self-declared)
2. Benchmark pubblici pricing UAV 25-40 kg MTOW classe — confidence low-medium (estremamente variabile)
3. Reseller markup tipici aerospace mid-tier — confidence medium (best practice)
4. Contratti pubblici disponibili (EMSA €30M / 4 anni, UK Home Office £1B / 3 anni) — confidence high MA non breakable per singolo asset
5. Esperienza analogica con classi simili UAV mercato EU — confidence medium

**Le stime NON includono:**
- Sconti volume eventuali (acquisto multi-piattaforma Y2-Y3)
- Negoziazione condizioni payment (es. anticipo ridotto, milestones favorevoli)
- Componenti opzionali avanzate non incluse (es. EW, SAR radar per Tekever)
- Costi indiretti Firmamento (PM dedicato, legale specialistica, viaggi)
- Inflation aerospace 2026-2030 (tipicamente 3-5%/anno)
...
| Aspetto | Confidence |
|---|---|
| Specifiche tecniche datasheet vendor | M-H (datasheet vendor, ma non validati operatori EU indipendenti per JOUAV; validati per Tekever via reference EU) |
| Stima CapEx Y1 (range) | L-M |
| Stima TCO 5 anni | L (richiede assumptions su utilization + escalation prezzi) |
...
| Risk geopolitico (probabilità eventi) | M (analisi base rate + trend osservati 2024-2026) |
...
| **W26 (~M+6)** | **DECISIONE FINALE + INIZIO PRODUZIONE VENDOR** | **Avvio fornitura** | |
...
**Sintesi:**
- **6 mesi** da emissione RFQ a contract signed (best case, vendor EU intra-EU)
- **8-9 mesi** da emissione RFQ a contract signed (worst case, vendor CN con Golden Power)
- **+6-10 mesi addizionali** per produzione + delivery + SAT
- **Operatività Y1 avvio realistica: M+12-18** dalla decisione di emettere RFQ
...
### 9.3 Regulatory + market sources

- EASA Reg. UE 2019/947 + 2019/945 Specific Category
...
- D.L. 21/2012 Golden Power + modif. D.L. 23/2020 + D.L. 50/2022 (settore aerospace)
- NIS2 Directive 2022/2555 + DO-326A awareness
...
- US drone tariffs 2025-2026 (Section 301 + 122) — https://uavcoach.com/drone-tariffs/
...
- `vtol-uas-specialist` (database piattaforme + pricing benchmark) — `.claude/agents/vtol-uas-specialist.md`
- `trade-study-analysis` (matrice decisionale ponderata) — `.claude/skills/`
- `epistemic-rigor` (caveat + falsifying observations + confidence calibration) — `.claude/skills/`
- `competitor-intelligence` (analisi vendor + reference customers) — `.claude/skills/`
- `regulatory-adversary` (compliance EU/IT + Golden Power) — `.claude/skills/`
...
| Engagement plan vendor | ✅ timeline 3-6 mesi documentata + decision review board |

**DR-003 chiusura completa richiede:** invio RFQ formale + ricezione quotation reali entro M+6-7.
...
2. **Personalizzare cover letter** (`RFQ-cover-letter-template.md`) per ogni vendor target
3. **Inviare RFQ in parallelo** a JOUAV (via reseller EU) + Tekever + Quantum F90+ (terzo per benchmark) entro M+4
...
Quando quotation reali sono ricevute (M+6 atteso):
1. Aggiornare matrice decisionale §6.2 con dati reali (sostituire stime)
2. Validare/falsificare FO-1 a FO-8 in §7.2
3. Decidere se conservare baseline JOUAV o swap a Tekever
4. Avviare Golden Power notifica formale (se JOUAV vince) o procedere a contract drafting Tekever (se vince)
5. Comunicare al Board Firmamento la decisione finale + budget approvato + cronoprogramma
