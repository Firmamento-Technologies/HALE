...
Questo documento è il **Livello 2 (Detailed ICD)** della roadmap di versioning definita in A4-ICD-PRELIMINARE-v1.0 §A.4.8, prodotto in anticipo rispetto alla milestone nominale M+10-M+12 a seguito del rilievo Red Team. Le 5 interfacce trattate sono quelle identificate come **flight-critical o safety-critical** per il Percorso 6A VTOL Pentema BVLOS.
...
| **DAL equivalente** | DAL-B (DO-178C / ARP4754A) — failure → Hazardous |
...
| **Status** | **Detailed** |
| **Confidence** | **medium** — specifiche MAVLink consolidate; comportamento FCS JOUAV proprietario non ancora documentato da vendor |
...
**Note L2:** Frame TDD con ciclo 10 ms: 7 ms downlink (telemetry + video), 3 ms uplink (comandi). CRC-X25 (CRC-16/MCRF4XX — polinomio 0x1021, init 0xFFFF) calcolato su intero MAVLink v2.0 packet (header + payload), come definito in MAVLink Common Message Set §2.4 [^MAVLink].
...
**Requisiti nonce:**
- Nonce non deve mai essere riutilizzato con la stessa chiave (AES-256-GCM nonce-reuse vulnerability)
- SEQUENCE_48 incrementato monotonicamente; wrap-around reset forza re-keying
- Re-keying obbligatorio ogni **2^32 frame** (~4 miliardi pacchetti — ~50 giorni a 1 kHz) o ogni **24 ore operative** (whichever first)
...
**GCM Tag verification:** il ricevitore verifica il tag a 128 bit prima di processare il comando. Packet con tag non valido: scartato silenziosamente + incremento contatore CRYPTO_FAILURE_COUNT; se CRYPTO_FAILURE_COUNT > 10 in 1 s → log STATUSTEXT severity=ERROR + alert GCS.
...
|---|---|---|---|---|
| LINK_OK | heartbeat age ≤ 2 s | — | Normale | — |
...
| LINK_LOST | heartbeat age > 5 s consecutivi | — | **RTL automatico** (MAV_CMD_NAV_RETURN_TO_LAUNCH) | CRITICAL + SMS PC |
| EMERGENCY_LAND | RTL fail o fuel/battery critical | — | **Land in situ** (MAV_CMD_NAV_LAND) su pre-computed safe zone | CRITICAL + 406 MHz ELT |
...
t=5s+60s-120s: stimata rientro (dipende da distanza, ≈ 5-10 km/min)
...
In shadow zone orografiche (previste in Val Trebbia / Val Pentemina), il link può presentare:
- RSSI < -85 dBm → PER (Packet Error Rate) > 20% → heartbeat intermittenti
- Scenario: 3 heartbeat consecutivi mancati su 5 (entro finestra 5 s) → LINK_LOST trigger

Per evitare false LINK_LOST in condizioni di link degradato, il FCS usa il criterio:
...
| Demodulazione UAV | 1 ms | — |
| Decifrazione AES-GCM + CRC verify | 2 ms | — |
...
- In NLOS, multipath introduce delay spread fino a 5 µs (trascurabile a L7)
- OFDM cyclic prefix 800 ns copertura: sufficiente per delay spread < 800 ns
- Se NLOS severo: link margin scende sotto 6 dB → BER > 10^-3 → ARQ L2 → latency +10–50 ms
- Budget worst-case NLOS: ~80 ms one-way — ancora entro 250 ms
...
- MAVLink Common Message Set v2.0 dialect [^MAVLink]
- RTCA DO-377A — Minimum Operational Performance Standards for C2 Link Systems [^DO377]
- DO-326A / ED-202A — Airworthiness Security Process Specification [^DO326A]
- EN 300 328 v2.2.1 — WLAN/ISM 2.4 GHz harmonized standard EU
- NIST SP 800-38D — Recommendation for GCM (AES-GCM) [^NIST38D]
- JARUS SORA 2.5 Annex F §4.2 — Lost-link procedure requirements
...
5. L-band (1616–1626.5 MHz) ha attenuazione rain fade < 1 dB @ Liguria (ITU-R P.618-14) — affidabilità > 99.9%

**Limitazione nota:** latency Iridium 1.5 s supera il limite DO-377A §3.4.1 di 250 ms per pilot-in-the-loop. Pertanto SATCOM non è usato per manovre in tempo reale; è usato esclusivamente per:
- Heartbeat monitoring (conferma UAV alive durante RF gap)
- RTL command invio (non time-critical una volta deciso)
- Telemetria di sicurezza bassa frequenza (1 Hz)
...
**Nota TCP vs UDP:** a differenza del link RF primario (UDP, latency-optimized), il link SATCOM usa TCP per garantire delivery affidabile dei comandi safety-critical (RTL) in presenza di burst error L-band.

### [v2.0 NEW] MAVLink subset ridotto per SATCOM

Il bitrate Iridium 22 kbps uplink impone un subset di messaggi MAVLink:

**Downlink UAV→GCS via SATCOM (22 kbps uplink Iridium = ~2750 B/s — NOTA: uplink Iridium = UAV trasmette a satellite):**
...
**Messaggi esclusi da SATCOM** (bandwidth insufficiente o latency non adatta):
- ATTITUDE (10 Hz — troppo frequente)
- HIGHRES_IMU (10 Hz)
- RC_CHANNELS
- Video preview (qualsiasi)
...
Il modem Iridium 9770 (o compatibile Iridium Certus 100) è controllato via **UART 115200 baud 8N1** da Mission Computer. Sequenza di attivazione link SATCOM:
...
# Nessun comando AT necessario — routing layer software gestisce
# SATCOM rimane in standby (warm) per riattivazione rapida < 2 s
```

**Tempo di setup link da cold start modem:** ~30 s (acquisizione satellite Iridium, registration network)
**Tempo di setup da warm standby (modem acceso, non in sessione):** ~2–3 s
...
| GCS processing | 5 ms | — |
| **Totale one-way (UAV→GCS telemetry)** | **≈ 385 ms** | — |
...
| **DAL equivalente** | DAL-C (DO-178C) — failure → Mission abort, no safety hazard diretto |
...
| **Status** | **Detailed** |
| **Confidence** | **medium** — Sony GigE Vision consolidato; Workswell USB SDK well-documented; bus architecture finale dipende da selezione hardware MC |
...
→ OPPURE: full frame 1 fps + burst 3 fps in memoria buffer camera (buffer interno 828 MB per ~7 frame burst)
...
| PPS timing | GPIO | — | 1 pulse/s | 1 pulse/s | GPIO |
...
| DF19 | Military ADS-B Extended Squitter | — | Trascurabile (escluso zona) |
...
|---|---|---|
| Minimum range reception | 80 NM (148 km) @ FL180 | In area Pentema (bassa quota), ostacoli orografici riducono range effettivo |
...
**Contesto:** ENAC SORA 2.5 per SAIL III Pentema potrebbe richiedere DAA non-cooperativo per protezione da:
- Alianti, ULM, parapendio senza transponder (comuni in Val Trebbia)
- Elicotteri di soccorso in emergenza (Soccorso Alpino, 118)
...
| **Verdetto per Fase 1** | **Non selezionato** | — | — | — | — | **Confidence low; defer a ENAC feedback** |
...
**Falsifying observation:** se ENAC valuta che SAIL III Pentema richiede DAA non-cooperativo certificato DO-365B Annex C prima del first flight → hold operazioni BVLOS; costo addizionale €150-300k + 6-12 mesi lead time. Cross-ref: RSK-INT-003 (P=3, I=5, P×I=15 RED).
...
│                                                                         │
│  DO-365B §2.2.3 requirement: alert ≤ 35 s before CPA for TA           │
...
| **DAL equivalente** | DAL-D (DO-178C) — failure → compliance violation, no safety hazard diretto |
...
**Requisito GDPR:** il trattamento dati con riprese aeree è un trattamento a rischio elevato (EDPB Guidelines 01/2022 §3.2 [^EDPB]) → DPIA obbligatoria + audit trail verificabile.
...
| Failure Mode | Effetto | Categoria | Mitigation |
|---|---|---|---|
| YOLOv8n crash | Frame non blurred → GDPR violation | Compliance-Critical | Watchdog: se model fail → FULL FRAME BLUR (tutta immagine oscurata) + alert + abort mission |
...
- GDPR Reg. UE 2016/679 Art. 25 (Privacy by Design), Art. 32 (sicurezza), Art. 35 (DPIA)
- EDPB Guidelines 01/2022 on data subject rights (update 2023) [^EDPB]
- EDPB Guidelines 10/2020 on restrictions under Art. 23 GDPR
- D.Lgs. 196/2003 (Codice Privacy IT) come modificato da D.Lgs. 101/2018
- OpenCV 4.x (Apache License 2.0) — cv2.GaussianBlur, cv2.rectangle
- Ultralytics YOLOv8 (AGPL-3.0 / commercial license) — detection framework
- NIST SP 800-57 Part 1 Rev. 5 — Key Management (ECDSA P-256 signing)
...
| INT-13: DAA senza algoritmo | CHIUSO (cooperativo); PRELIMINARY (non-cooperativo — TRL constraint) |
...
| GAP-02 | INT-04 | Coverage test SATCOM Iridium in Valle Pentema (shadow zone orografiche) non ancora eseguito | RF Systems Engineer | M+11 (field test) | SATCOM non disponibile in shadow zone → RTL prematur |
