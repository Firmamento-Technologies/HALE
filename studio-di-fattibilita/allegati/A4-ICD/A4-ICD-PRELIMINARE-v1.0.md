...
L'ICD v1.0 è il **secondo livello di rigore** previsto dalla disciplina NASA SE Handbook §6.3 [^1]:
...
In coerenza con Cap. 0bis, Cap. 4 §4.0bis, Cap. 6 §6.0bis:
...
Criterio di criticità derivato da **DO-178C/DO-254 DAL allocation** (Development Assurance Level), applicato cross-cutting:
...
Il Percorso 6A si fonda su una piattaforma VTOL ibrida commerciale TRL 8-9 (baseline JOUAV CW-30E o alternative EU come Tekever AR3/AR5, Quantum Trinity F90+, FlyingBasket FB3). Le interfacce 6A sono prevalentemente **standard COTS aerospace consolidati**, con confidence prevalentemente medium-high.

### A.4.2.1 INT-6A-PHY-* — Interfacce fisiche velivolo (10 interfacce)

Le interfacce fisiche del Percorso 6A coprono integrazione meccanica/elettrica/termica payload-airframe, antenne RF, landing gear, alimentazione GS, backhaul internet.
...
| **⚠️ Falsifying observation** | Se il vendor JOUAV (o equivalente) **non fornisce CAD interfacce dettagliato** entro M+6 pre-engagement → necessario re-engineering custom mount adapter +€15-25k + lead time +2-3 mesi → impatto budget contingency +10-15%. |
...
| **Status** | Preliminary |
| **Confidence** | high |
| **Note** | Sub-interface di INT-02 Cap.4. GigE Vision è standard consolidato per camera industrial. Phase One iXM 100 supporta nativamente. |
...
| **⚠️ Falsifying observation** | In **valle Pentema shadow zones** orograficamente probabili (Val Trebbia, Val Pentemina) → range effettivo 50 km LOS si riduce a 20-30 km con NLOS. Mitigazione: **switch automatico a SATCOM L-band (INT-6A-C2-003) entro 5 s** di lost-link. Test campo M+11 (in autorizzazione condizionata) per misura effettiva orografia. |
...
| **⚠️ Falsifying observation** | Se RTH path attraversa **shadow zone GPS** (canyon profondo Pentema) o ostacolo orografico non mappato → emergency land può atterrare in **area non sicura** (centro abitato, strada). **Mitigazione obbligatoria**: 3 RTH waypoint intermedi pre-pianificati con altitude buffer ≥ 100 m AGL; emergency land zones identificate via DTM 10m Liguria (Vol. 2 Allegato A.16); flight envelope no-fly zone su centri abitati (geofence INT-6A-C2-008). |
...
Le interfacce regolatorie sono **gestite tramite formalismi documentali** (lettere PEC, application formali, DPIA, polizze assicurative). Sono **compliance-critical**: failure compromette autorizzazione operativa.
...
Il Percorso 6B (HALE solare stratosferico) è in stato **R&D Phase B preparatorio**. Le interfacce 6B hanno confidence prevalentemente **low/speculative** e sono dichiarate **Concept**: il dettaglio Detailed ICD è deferito a Phase C-D (M+48-M+60+).
...
| **⚠️ Falsifying observation** | RSK-TEC-001 cross-reference: se peso pannelli effettivo **> 0.8 kg/m²** (vs target 0.6) → MTOW +12-15 kg → **energy balance inverno deficit aggravato** del 20-30%; se efficienza effettiva **< 28%** (vs target 30%) → E_solar_day -10% → deficit margine inverno aggravato. Verifica supplier specs early M+12-M+18 (Fase 1+). |
...
|---|---|
| **Confidence** | **low** |
| **⚠️ Falsifying observation** | RSK-TEC-001 cross-reference (energy balance inverno deficit -50.1% post-simulazione M+3): se thermal management **heater richiede > 80 W continuous** (vs baseline 50 W) per mantenere batterie LiS in range +5/+45°C @ ambient -65°C → **energy balance notturno ulteriormente aggravato** → showstopper RSK-TEC-001 confermato → **abbandono perennial flight 44°N** → fallback "E5 Seasonal-only" (marzo-ottobre) → revisione business case Percorso 6B (riduzione revenue persistente, focus su seasonal services). Test bench inverno simulato Fase 3 M+38. |

### A.4.3.2 INT-6B-NTN-* — Service link 5G NR-NTN (4 interfacce)

Le interfacce service link NTN sono **frontier R&D**: 3GPP Rel-17/18 NTN solutions sono pubblicate ma deployment commerciale HAPS è in fase pre-operativa globalmente (2026).
...
| **⚠️ Falsifying observation** | Spettro S-band n255 è **contestato con MNO terrestri** (TIM, Vodafone) che hanno licenze IMT regionali → senza **accordi spectrum sharing** o **spectrum trading** Firmamento non può operare commercialmente in S-band. **Mitigazione**: (1) engagement TIM/Vodafone per partnership wholesale (Firmamento fornisce capacity, MNO eroga al cliente finale); (2) alternative banda HAPS dedicata C-band 6.4-6.7 GHz (post-WRC-19) — meno congestionata, ma con UE penetration più limitata. Roadmap regolatoria spettro: vedi Cap. 5 §5.5. |
...
Il feeder link Ka-band 27.9-31.3 GHz è **HAPS-dedicated post-WRC-19**: spettro disponibile globalmente per HAPS, ma coexistence con Fixed Satellite Service (FSS) GEO richiede coordinamento ITU.
...
|---|---|
| **Confidence** | **low** |
| **⚠️ Falsifying observation** | **Rain fade Genova Zona K (ITU-R P.618-14)** può raggiungere **25 dB @ 99.9% availability** in stagione invernale-primaverile → margine link richiesto significativo. Se EIRP HAPS non riesce ad eguagliare/superare questo margine + uplink interference da operatori vicini → **outage non accettabile per servizio commerciale wholesale** (SLA tipico telco ≥ 99.5%). **Mitigazione**: Adaptive Coding & Modulation (ACM) DVB-S2X mandatory; site diversity (2+ gateway separati > 30 km, es. Genova + La Spezia, statisticamente decorrelati); strategia operatività graceful degradation in eventi estremi. |
...
|---|---|
| **Confidence** | **low** |
| **⚠️ Falsifying observation** | **Pentema NON adatto come launch site HALE** (terreno montano, runway grass impossibile, accesso aerotraino impossibile). Site candidato realistico: **ex aeroporto Sarzana-Luni o aeroporto Albenga** (entrambi Liguria, ma con vincoli operativi propri). **Lead time site permitting ≥ 12 mesi** dopo Phase C-D Go decision. **Weather window stringente** (wind ≤ 5 m/s @ ground, no rain, clear sky per visibility) → operabilità launch/recovery ≤ 60-80 giorni/anno realisticamente in Liguria → necessario flotta multi-HALE per garantire copertura continua wholesale. |
...
| **Status** | Concept |
| **Confidence** | high |
| **Note** | Sub-interface di INT-20 Cap.4. Aruba ha **label GAIA-X confermato 2025**. OVH Italia e IONOS Italia pari opportunità. Hyperscaler USA (AWS/Azure/GCP) **NON GAIA-X compliant** per default → da escludere strategicamente per boundary B2 (sovereign infrastructure). |
...
|---|---|
| **Confidence** | **medium** |
| **⚠️ Falsifying observation** | Il vendor baseline **JOUAV (CN — Cina)** potrebbe essere soggetto a **export control issues** post-evoluzione geopolitica (tensioni EU-CN su dual-use, evolution Wassenaar Arrangement, eventuali sanzioni settoriali) → impossibilità di importare CW-30E o di ricevere spare parts/support. **Mitigazione obbligatoria**: identificare **vendor alternativi EU pre-RFQ M+6**: (1) **Tekever AR3/AR5** (Portogallo, EU AIRBUS partnership), (2) **Quantum Systems Trinity F90+** (Germania, dual-use cleared), (3) **FlyingBasket FB3** (Italia, supply chain europea). Decisione vendor finale con criterio supply chain risk + geopolitical resilience ponderato in DOCFAP M+8. Vedi `riferimenti/RESERVED-rischi-geopolitici.md` per analisi geopolitica completa. |

---

## A.4.5 Matrice compatibility interfacce

La **Matrice di Compatibilità** valuta interfacce-interfacce potenziali conflitti (mass+power+thermal+EMC) o sinergie. È strumento di **integration engineering**: identifica accoppiamenti dove un parametro di un'interfaccia (es. potenza payload) impatta un'altra (es. battery sizing).

### A.4.5.1 Matrice condensata (10×10 critical 6A)

Per leggibilità, riporto qui solo la matrice condensata su 10 interfacce 6A critiche. La matrice completa (50×50) è nel foglio Excel `ICD-v1.0.xlsx → Compatibility_Matrix`.

| | PHY-001 | PHY-002 | PHY-004 | PHY-005 | PHY-006 | PHY-007 | PHY-009 | PHY-010 | C2-001 | C2-003 |
|---|---|---|---|---|---|---|---|---|---|---|
| **PHY-001** Payload Bay | — | OK | OK | OK | OK | Check (mass+thermal) | OK | OK | OK | OK |
| **PHY-002** 28V Power | OK | — | OK | OK | OK (BMS) | OK | OK | OK | OK | OK |
| **PHY-004** Ground Ant | OK | OK | — | OK (RF coupling) | OK | OK | OK | OK | OK (RF feeder) | OK |
| **PHY-005** Whip Ant | OK | OK | OK (RF coupling) | — | OK | OK | OK | OK | OK | OK |
| **PHY-006** Battery | OK | OK (BMS) | OK | OK | — | Check (thermal) | OK | OK | OK | OK |
| **PHY-007** Thermal | Check (mass+thermal) | OK | OK | OK | Check (thermal) | — | OK | OK | OK | OK |
| **PHY-009** GS Power | OK | OK | OK | OK | OK | OK | — | OK | OK | OK |
| **PHY-010** Backhaul | OK | OK | OK | OK | OK | OK | OK | — | OK | OK |
| **C2-001** RF Uplink | OK | OK | OK (RF feeder) | OK | OK | OK | OK | OK | — | OK (fallback) |
| **C2-003** SATCOM | OK | OK | OK | OK | OK | OK | OK | OK | OK (fallback) | — |
...
1. **(PHY-001, PHY-007) Mass + Thermal**: il payload bay deve sostenere mass budget + thermal dissipation. Se payload genera >80 W heat e operating range stringente, il design termico (heat path + NACA cooling) deve essere validato congiuntamente al design meccanico.
2. **(PHY-004, PHY-005) RF Coupling Antenna**: ground antenna e airborne antenna devono operare nello stesso link RF: pattern + polarizzazione + EIRP + sensitivity must be co-designed.
3. **(PHY-006, PHY-007) Battery Thermal**: la batteria genera heat in discharge + assorbe heat in charge. Thermal management influisce su SOC sensing accuracy e safety (thermal runaway prevention).
4. **(C2-001, C2-003) Primary + Secondary Link Switch**: la logica di switch automatico tra RF 2.4 GHz e SATCOM L-band deve gestire seamlessly (no perdita comando in transizione). Test critico in field test.
...
**Osservazione**: il **profilo di rischio interfacce 6B è strutturalmente più alto** del 6A — coerente con stato R&D Phase B preparatorio e confidence prevalentemente low. Il Risk Register è dinamico: aggiornamento M+6 + M+9 + M+11 con trend P×I (target downward).

---

## A.4.7 Test plan integrazione interfacce

Il Test Plan v1.0 identifica **20 test campaign** correlate alle interfacce dell'ICD, in 4 fasi:
...
L'ICD v1.0 è la **baseline preliminary**. Versioni successive aggiungeranno detail e validazione progressiva:
...
L'ICD viene re-baselined se occorre uno dei seguenti **trigger eventi**:
...
1. **INT-6A-PHY-001** — Se vendor JOUAV (o eq.) non fornisce CAD interfacce → re-engineering custom +€15-25k
...
7. **INT-6B-PHY-003** — Se thermal management heater > 80W → energy balance inverno aggravato → showstopper
...
L'ICD è strettamente accoppiato con:
