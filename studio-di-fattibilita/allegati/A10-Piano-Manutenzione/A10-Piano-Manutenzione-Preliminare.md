...
Il Piano di Manutenzione preliminare definisce le **procedure di manutenzione programmata + correttiva** per il sistema HALE/VTOL Firmamento, in coerenza con:
...
|---|---|---|---|
| **Pre-flight check** | MNT-PRE | Verifica visiva + funzionale prima di ogni volo | Ogni missione |
| **Post-flight check** | MNT-POST | Ispezione + download log dopo volo | Ogni missione |
| **Daily / Weekly** | MNT-DAILY | Procedure giornaliere/settimanali | Quotidiana operativa |
...
- Battery charging cycle management (LiPo 50-90% optimal storage)
- Software update FCS + GS (su patch release vendor)
- Cybersecurity patch (Reg. UE 2023/203 Part-IS)
- Calibration sensori IR (se utilizzati nella settimana)

### MNT-50H — Major check ogni 50 ore (4-6 settimane in MVP intensivo)

- Smontaggio + ispezione motori VTOL (4 unità + cruise)
- Sostituzione bearings se usurati
- Ispezione + sostituzione eliche se danneggiate
- Ispezione + cleaning sistema raffreddamento motori
...
- Smontaggio completo + ispezione strutture primarie (longheroni, ala, fusoliera)
...
- Audit completo logbook + storia operativa
- Compliance check (SORA, dichiarazioni ENAC, assicurazione)
- Re-training piloti UAS (mandatory ENAC + EASA)
- Audit Part-IS Information Security (Reg. UE 2023/203)
- Renewal certificazioni operatore
...
- Telemetria health monitoring (continuous)
- Anomalie detection + alert automatici
- Pianificazione discese opportunistic per maintenance (~ 1 mese cycle)
...
Per il MVP Y1 (Percorso 6A):
- **In-house** (3 FTE: pilota + ingegnere + tecnico)
- Supporto vendor per MNT-200H (annual, contratto SLA)
- Certificazione AS/EN 9110 in corso (target M+12-18)

Per scale-up Y3+ (multi-regione):
- Dedicated MRO facility (Pentema o sito ottimizzato)
- 5-8 FTE manutenzione
- Vendor partnership per overhaul + ricambi

Per Phase B 6B HALE:
- Cooperazione con prime contractor (post DR-014 pivot)
- MRO facility specialized stratospheric ops
- Partnership CIRA/Polito per test + validation
...
- **Aircraft Logbook** (per ciascun velivolo) — vita operativa + tutte le manutenzioni
- **Component Logbook** (per ciascuna parte critica) — tracciabilità lifetime
- **Maintenance Records** (database digitale) — query + audit + report

Conservazione: **almeno 5 anni** post-disposal velivolo (requisito ENAC).

## A.10.8 Falsifying observations

- Se MTBF reale Y1 < 100 h (vs 200 h target SyR del Cap. 3), il Piano di Manutenzione va rivisto al rialzo + lifetime motori/batterie da rinegoziare con vendor
- Se Part-IS Reg. UE 2023/203 audit identifica gap, CISO + remediation entro 30 gg

## A.10.9 Versioning

- v1.0 M+3 (presente, preliminare)
- v1.5 M+6 — Update con piloti reali + vendor manuals
- v2.0 M+10 — Maintenance Plan definitivo per gate G3 + SORA application
