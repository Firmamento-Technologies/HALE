...
> Confidence aggregato: **MEDIUM**. Il documento richiede validazione con manuale tecnico OEM JOUAV CW-30E
...
Il PMO supporta la pre-application ENAC SORA SAIL II-III BVLOS, il calcolo dei costi operativi LCC per Cap. 8 §8.5.1 e la conformità EN 9110 per l'organizzazione di manutenzione.
...
Caveat: la certificazione formale EN 9110 da parte di organismo terzo accreditato (es. ACCREDIA) non costituisce requisito ENAC obbligatorio per operatore UAS Specific Category Y1, ma resta fortemente raccomandata come evidenza di competenza OSO #03 (SORA 2.5) e come prerequisito di contratti PA.
...
- L'operatore Specific Category mantiene il SAPR in condizioni di aeronavigabilità
...
- Per operazioni BVLOS Specific, ENAC può richiedere dimostrazione del programma di manutenzione nell'ambito della SORA application

Impatto PMO: il Piano di Manutenzione è documento obbligatorio da allegare alla dichiarazione ENAC Specific + SORA application. Il presente PMO v1.5 costituisce l'evidenza iniziale; la versione v2.0 (M+10) sarà quella allegata alla SORA application definitiva.

#### 1.2.4 Reg. UE 2019/947 + 2019/945, Conformità operativa e aeronavigabilità

Il Reg. UE 2019/947 (operazioni UAS), artt. 13-16 (Specific Category):
- Art. 14: operatore responsabile della manutenzione del SAPR
- Art. 15: obblighi di notifica incidenti e gravi inconvenienti all'autorità nazionale (ENAC)
- Artt. 16-17: procedure operative documentate (include manutenzione pre-post flight)

Il Reg. UE 2019/945 (requisiti tecnici prodotti UAS) definisce i requisiti di costruzione ma non istituisce un tipo di certificazione obbligatoria per Class III operato in Specific Category. La Declaration of Conformity del costruttore (JOUAV) resta evidenza rilevante per OSO #02 e OSO #04.

#### 1.2.5 ISO 14001:2015, Gestione ambientale

La ISO 14001 si applica alla gestione ambientale delle attività di manutenzione, con focus su:
...
La manutenzione preventiva (Preventive Maintenance, PM) viene pianificata secondo intervalli fissi (calendario o ore volo) a prescindere dallo stato osservato del componente. Per il CW-30E in Y1 (50-80 missioni, missione media 2-3 ore, totale ore stimate 100-240 h/anno) si adottano i seguenti intervalli PM:

- **MX-PRE/POST**: ogni volo (obbligatorio, non derogabile)
- **MX-DAILY**: ogni giorno operativo (anche senza voli, se velivolo in hangar)
...
Nota orografia Pentema: le missioni in ambiente alpino con vento canalizzato, turbolenza e temperature sotto zero aumentano il tasso di usura strutturale e dei motori del 15-25% rispetto a condizioni nominal sea-level. Gli intervalli PM vanno pertanto considerati come **massimi**, con possibilità di anticipazione on-condition se l'health monitoring rileva degradazione.

### 2.3 Manutenzione correttiva (CM)

La manutenzione correttiva (Corrective Maintenance, CM) si attiva da:
...
Processo CM:
1. Apertura Non-Conformity Report (NCR) nel sistema di tracciabilità
...
Le ispezioni straordinarie si attivano dopo:
...
L'overhaul (MX-OVH) viene pianificato per i componenti con life limit stabilito da OEM o da analisi on-condition:

| Componente | Life limit stimato | Trigger overhaul |
|---|---|---|
| Motori VTOL (4 unità) | 200-400 h o 2 anni (valore vendor da confermare) | Raggiungimento limite o degradazione misurata |
| Motore cruise (1 unità) | 300-500 h o 2 anni | Come sopra |
| Pack batteria LiPo propulsione | 300-500 cicli o 24-36 mesi | Capacità residua < 80% nominale |
...
Caveat: i valori di life limit derivano da benchmark di mercato UAS commerciale (medium confidence). Il manuale tecnico OEM JOUAV CW-30E (da acquisire al M+6 con RFQ) potrebbe stabilire limiti differenti, che prevalgono sui presenti.
...
- **Durata**: ore stimate per attività; includono setup strumenti ma non logistica parti
...
| ID Task | Descrizione attività | Freq | Strumenti | Pers | Durata (h) | Costo mat (€) | Costo MdO (€) | Rif. OEM |
|---|---|---|---|---|---|---|---|---|
...
| MX-PROP-014 | Capacity test batteria LiPo (discharge controllato, misura capacità reale) | MONTHLY | Battery analyzer / discharge tester | ML1 | 1.5 | 0 | 45 | JOUAV TBD §X.X |
...
- Le temperature invernali (-10°C) riducono la capacità delle batterie LiPo del 20-35% rispetto a 25°C. Procedura obbligatoria: preriscaldamento batterie a minimo +10°C prima di qualsiasi carica o utilizzo. Non caricare LiPo a temperature inferiori a 5°C.
- La quota (1.100-1.300 m s.l.m.) implica densità aria ~87% rispetto a sea level. I motori VTOL devono compensare con maggior corrente assorbita (+10-15%); va monitorata la temperatura motori durante le prime missioni invernali.
...
| ID Task | Descrizione attività | Freq | Strumenti | Pers | Durata (h) | Costo mat (€) | Costo MdO (€) | Rif. OEM |
|---|---|---|---|---|---|---|---|---|
| MX-AVI-001 | Verifica lock GPS + qualità segnale (DOP, num. satelliti >= 8) | PRE | GCS display | PIC | 0.05 | 0 | 3 | JOUAV TBD §X.X |
...
| MX-AVI-017 | Audit Part-IS: verifica log accessi, aggiornamenti sw, vulnerabilità note | ANN | Checklist CISO + log system | ML2 + CISO | 2.0 | 0 | 200 | JOUAV TBD §X.X |
...
| MX-STR-002 | Verifica integrità giunzioni rapide ala-fusoliera, boom-gondola | PRE | Torcia, mano (gioco residuo) | PIC o ML1 | 0.1 | 0 | 5 | JOUAV TBD §X.X |
...
| MX-PLD-007 | Download e verifica integrità dati missione (EO + IR, formato + geotag) | POST | Laptop + software Workswell + lightroom/Metashape | ML1 | 0.2 | 0 | 10 | Workswell + JOUAV |
...
| MX-PLD-016 | Verifica integrità storage interno camera (SD card o SSD) | WEEKLY | Laptop + utility disk | ML1 | 0.1 | 0 | 5 | Sony manuale |
...
| MX-RF-011 | Verifica conformità spettrale (AGCOM LRA 5.16.13): test con spectrum analyzer | ANN | Spectrum analyzer portatile o laboratorio | ML2 + RF consultant | 4.0 | 0 | 400 | AGCOM LRA |
...
- **Periodo operativo**: M+6 al M+18 (primi 12 mesi di operativi, contati da inizio operazioni effettive)
...
Per semplicità di pianificazione, si assume il **calendario 100-hour inspection a M+5 e M+10** dall'inizio operazioni (corrispondente a ~80 missioni x 2.5 h media = 200 h totali, quindi due 100H inspection durante Y1 nel caso ottimistico).
...
| Attività | Frequenza | Ore/evento | Ore/anno stimate |
|---|---|---|---|
...
Implicazione personale: con 1 maintainer part-time FTE 0.5 (circa 800-900 h/anno disponibili), il carico di 304-340 h è gestibile con un 40% del tempo del maintainer dedicato alla piattaforma. Il restante 60% copre GCS, payload, formazione e documentazione.

### 4.4 Stagionalità Pentema
...
Per Y1 (startup operations) l'organizzazione di manutenzione resta **interna a Firmamento Technologies**, con supporto esterno per attività specialistiche (NDT, RF, OEM). Non si prevede il conseguimento della certificazione EN 9110 formale in Y1; il target è M+18-24.
...
- Apertura/chiusura NCR, gestione non-conformità
...
- MX-DAILY, MX-WEEKLY (attività di routine non invasive)
- Riconoscimento anomalie e apertura NCR preliminare

Task autorizzati ML1: solo attività di ispezione visiva e funzionale; nessuno smontaggio/sostituzione componente critico senza supervisione ML2.

#### 5.2.4 Training plan Y1

| Attività formativa | Target | Timing | Fornitore | Costo stimato |
|---|---|---|---|---|
...
Il contratto di servizio OEM costituisce un elemento critico della strategia di manutenzione Y1. Si raccomanda di negoziare con JOUAV (o distributore EU/IT) al momento della RFQ (M+6) un contratto che includa:
...
- **Spare parts supply agreement**: impegno vendor a garantire disponibilità ricambi critici per 5 anni dal contratto, con lead time massimo concordato (target < 30 giorni per ricambi critici, < 14 giorni per consumabili)

**Caveat supply chain**: come indicato in RSK-GEO-003 (Allegato A.2 Risk Register), la supply chain JOUAV (vendor CN) resta soggetta a rischi geopolitici. Il contratto OEM deve prevedere clausola di **stock minimo di emergenza** e identificazione di fornitori alternativi per i componenti non proprietari (es. motori brushless, batterie LiPo, antenne). Vedi Sezione 6 per dettaglio.

### 5.4 Subcontractor specialistici

| Subcontractor | Attività | Frequenza prevista | Costo stimato/anno |
|---|---|---|---|
| NDT Service (ispezione compositi) | MX-STR-006 (100H + post-impatto) | 1-2 volte/anno | €1.500-3.000 |
| RF/spettrale Laboratory | MX-RF-011 (conformità spettrale AGCOM) | 1 volta/anno | €1.000-2.500 |
...
| Componente | Q.tà | Motivo criticità | Costo unit. stimato (€) | Totale (€) | Lead time JOUAV | Fonte prezzo |
|---|---|---|---|---|---|---|
| Motore brushless VTOL completo (set 4) | 4 unità | Alta usura + lead time CN lungo | 300-500 | 1.200-2.000 | 4-8 settimane | Benchmark mercato [vendor] |
...
| Modulo ESC (2 spare) | 2 unità | Failure ESC = fermo piattaforma | 150-300 | 300-600 | 4-8 settimane | Benchmark [vendor] |
| Modulo GPS/RTK (1 spare) | 1 unità | Failure GPS = no-go BVLOS | 400-800 | 400-800 | 6-10 settimane | Benchmark [vendor] |
| Antenna velivolo C2 (2 spare) | 2 unità | Danno fisico frequente | 30-100 | 60-200 | 1-2 settimane | Mercato generico |
...
| Componente | Q.tà | Fornitore possibile EU | Costo unit. (€) | Totale (€) |
|---|---|---|---|---|
...
Impatto finanziario Piano B: sostituzione piattaforma €150-300k aggiuntivi (CapEx non pianificato). La copertura proviene dalla contingency Y1 (15% previsto in Cap. 8 §8.4.1) o dalla rinegoziazione funding Cooding Prototypes.
...
Ogni velivolo JOUAV CW-30E dispone di un Aircraft Logbook (cartaceo + digitale) che registra:
...
- Condizioni meteo (vento, temperatura, visibilità)
...
- Firma Maintenance Manager (per attività 25H+)
- Maintenance Release (CRS, vedi §7.3)

### 7.2 Component Logbook

Per i seguenti componenti critici (life-tracked) si mantiene un Component Logbook separato:
...
| Pack batteria propulsione (x2) | Cicli carica totali, capacity attuale (%) | < 80% capacità nominale |
| Pack batteria payload (x2) | Cicli carica totali, capacity attuale | < 80% capacità nominale |
...
- Dichiarazione di conformità ai requisiti del Piano di Manutenzione
- Firma e data Maintenance Manager
- Validità CRS (data rilascio + prossima scadenza manutenzione programmata)

Il CRS viene archiviato nel fascicolo velivolo e resta disponibile per ispezione ENAC.

### 7.4 Non-Conformity Report (NCR)

Ogni anomalia, malfunzionamento o non-conformità rispetto al Piano di Manutenzione viene documentata in un NCR, che include:
...
### 7.5 Sistema di tracciabilità digitale

**Strumento target Y1**: sistema di manutenzione digitale (possibile utilizzo di piattaforme open-source come **Flightlog.io**, **AirManager** o simili, oppure sistema custom Excel/Sheets con controllo versione) con backup locale cifrato + cloud (AgID PSN o equivalente per dati PA se il sistema viene usato come evidenza per contratti PA).
...
Come richiesto da Part-IS EASA (RSK-REG-019, Cap. 5 §5.16.4), il sistema di manutenzione mantiene:
...
- Audit annuale CISO della sicurezza del sistema di tracciabilità

### 7.7 Notifica incidenti ENAC/ANSV

In caso di incidente o grave inconveniente (ai sensi Reg. UE 2019/947 art. 15 + D.Lgs. 213/2009 per ANSV), la procedura prevede:
...
I costi rappresentano **stime di benchmark** (confidence: medium), non quotazioni fornitore. Fonte: benchmark operatori UAS commerciali EU, datasheet vendor, costi di mercato IT per manodopera tecnica aerospaziale. I valori definitivi richiederanno conferma al M+6 con RFQ vendor + offerte subcontractor.
...
| Sistema tracciabilità digitale (setup + licenze Y1) | 1.000-5.000 | Open source + personalizzazione o SaaS |
...
**Raffronto Cap. 8 §8.5.1.A**: il range €47.500-96.500 risulta compatibile con il benchmark Cap. 8 (€30-60k manutenzione pura + personale aggiuntivo manutenzione non incluso in quel baseline). La differenza è principalmente imputabile alla voce "personale manutenzione" (€25-45k), che in Cap. 8 §8.5.1.A appare parzialmente nella voce "Personnel" aggregata. Nessuna incongruenza; il PMO fornisce una lettura analitica.
...
| **Y4** | Maturità (flotta stabile + HALE prep) | €10.000-20.000 | €200.000-350.000 | **€210.000-370.000** | Overhaul piattaforme Y1 |
| **Y5** | HALE + VTOL (flotta 5+ + subscale HALE) | €15.000-30.000 | €250.000-400.000 | **€265.000-430.000** | Include manutenzione HALE subscale |

**Nota Y2-Y3**: il +30% OpEx/anno per manutenzione (dichiarato in Cap. 8 §8.5.2) risulta coerente con la crescita di flotta. Il driver principale: il personale (scala quasi linearmente con numero piattaforme) più che i contratti OEM (economie di scala parziali).

**Nota Y4**: l'overhaul piattaforme Y1 (motori, batterie) genera un CapEx una tantum di €15-40k per piattaforma secondo i life limit (200-400 h per motori, 300-500 cicli per batterie), che cade presumibilmente in Y2-Y3 per i componenti più sollecitati.

### 8.5 Sensitività LCC

Tre driver di incertezza principale:

1. **Lead time ricambi JOUAV**: se il lead time supera 8 settimane (rischio supply chain CN), il costo di scorte Livello 1 aumenta del 50-100% (maggior volume spare richiesto).
2. **MTBF reale motori VTOL**: se l'MTBF reale scende sotto 100 h (vs 200-400 h OEM dichiarato), le sostituzioni motori aumentano l'OpEx di €8.000-20.000/anno (4 motori x €400-500 x 2-4 sostituzioni/anno).
3. **Retribuzione maintainer IT**: il range ML2 a €40-60/h risulta congruente con il mercato IT 2025 per tecnici aerospaziali junior (fonte: benchmark Almalaurea 2024 + Metalcontract CCNL aeronautica). Un aumento del 15% impatta l'OpEx di €3.750-6.750/anno.
...
**Classificazione ATEX hangar Pentema**: la zona di ricarica e stoccaggio batterie LiPo richiede classificazione secondo ATEX Direttiva 2014/34/UE. La classificazione (Zona 0/1/2) dipende dalla concentrazione di vapori di solventi organici e gas (H2 in caso di thermal runaway LiPo) e va effettuata da tecnico abilitato (Responsabile del Servizio di Prevenzione e Protezione, RSPP). Target: classificazione Zona 2 con adeguate misure di mitigazione.
...
**Sostanze REACH**: verifica preliminare dei componenti acquistati per assenza di SVHC (Substances of Very High Concern) nella Candidate List ECHA. Responsabilità: il Maintenance Manager verifica le Schede di Dati di Sicurezza (SDS) dei fornitori.

### 9.3 Gestione avifauna, comportamento in area maintenance

Il sito di Pentema si colloca in area appenninica con presenza di rapaci (poiane, falchi, nibbi, possibilmente aquile su crinali vicini). Le misure per area manutenzione esterna (parking + prep area):
...
La presente sezione elenca i gap critici da chiudere prima dell'avvio operazioni Y1. Tali gap costituiscono anche condizioni di ingresso (entry criteria) per il gate G2 (M+6) e G3 (M+10).
...
|---|---|
| **Gap ID** | GAP-MX-001 |
| **Descrizione** | Il manuale tecnico OEM JOUAV CW-30E completo (procedures manutenzione, life limits componenti, torque values, wiring diagrams) non risulta disponibile pubblicamente. Costituisce condizione necessaria per finalizzare il PMO v2.0. |
| **Impatto** | Senza manuale OEM: gli intervalli di ispezione (25H, 100H) restano stime di benchmark non validate; i life limit motori/batterie non sono certificati; la firma ML2 su CRS non ha fondamento documentale OEM. |
...
|---|---|
| **Gap ID** | GAP-MX-002 |
| **Descrizione** | A M+3 (data presente PMO) non risulta ancora assunta la figura di Maintainer ML2 dedicato. La posizione è critica: senza ML2 le ispezioni 25H e 100H non possono essere condotte internamente. |
| **Impatto** | Senza ML2 le attività manutenzione dipendono interamente da OEM JOUAV (costoso e lento) o subcontractor non qualificato. Rischio: impossibilità di tenere la cadenza di 1-2 missioni/settimana per mancanza di clearance manutenzione. |
...
|---|---|
| **Gap ID** | GAP-MX-003 |
| **Descrizione** | Il contratto SLA con JOUAV (o distributore EU) per supporto tecnico remoto, visite annuali e spare parts supply resta da negoziare. Senza questo contratto il supply chain risk RSK-GEO-003 non viene mitigato. |
| **Impatto** | Senza SLA: il lead time ricambi non risulta garantito; nessun impegno OEM su disponibilità parti 5 anni; nessuna priorità su supporto tecnico. In caso di failure componente critico (FCS, motore) l'aeromobile può restare fermo 8-16 settimane. |
| **Azione richiesta** | Negoziazione SLA contestuale alla RFQ piattaforma (M+6). Clausole minime: lead time ricambi < 30 gg (critici) / < 14 gg (consumabili); supporto remoto SLA 48h risposta; impegno disponibilità ricambi 5 anni; 1 visita on-site/anno inclusa. |
...
|---|---|
| **Gap ID** | GAP-MX-004 |
| **Descrizione** | Il GSE (Ground Support Equipment) per manutenzione non risulta ancora acquisito. Senza gli strumenti adeguati (chiavi torsiometriche calibrate, multimetro precision, NanoVNA, oscilloscopio, battery analyzer, kit riparazione compositi) le ispezioni non possono essere condotte a standard. |
...
|---|---|
| **Gap ID** | GAP-MX-005 |
| **Descrizione** | L'hangar Pentema (80-150 m² previsto) richiede adeguamento per le attività di manutenzione: area separata manutenzione (min 20 m²), illuminazione tecnica, bench work, armadietti ATEX per LiPo, scaffalature ricambi, ventilazione meccanica area ricarica. |
...
2. **Se la capacity batterie LiPo scende sotto 80% nominale prima di 200 cicli**, la frequenza MX-PROP-014 (capacity test) va aumentata a ogni 25 cicli; il budget sostituzione batterie Y1 aumenta di €1.600-3.000. Trigger: primo capacity test < 80%.

3. **Se VVF Torriglia classifica l'hangar come ATEX Zona 1** (invece di Zona 2), i costi di adeguamento aumentano di €20-40k (vedi A.9 Computo Metrico). Trigger: classificazione ATEX formale entro M+6.

4. **Se il manuale OEM JOUAV CW-30E stabilisce intervalli diversi** (più frequenti) rispetto alle stime di benchmark qui utilizzate, il carico manutentivo annuo può aumentare del 20-50% e il PMO v2.0 deve ricalcolare tutti i costi LCC. Trigger: ricezione manuale al M+8.

5. **Se la supply chain JOUAV resta bloccata per oltre 60 giorni**, attivare Piano B Tekever AR3 (§6.6) e sospendere operazioni CW-30E. Trigger: ritardo fornitura ricambio critico Livello 1 > 60 giorni.
...
> Motivazione: (1) gli intervalli manutenzione si basano su benchmark mercato UAS, non sul manuale OEM JOUAV CW-30E (non disponibile pubblicamente); (2) i costi LCC derivano da stime di benchmark IT 2025, non da quotazioni ferme; (3) la vita utile componenti proviene da datasheet vendor non validati da operatori EU; (4) la classificazione ATEX hangar non è ancora effettuata da tecnico abilitato. Tutti e quattro i gap saranno chiusi tra M+6 e M+9 (vedi Sezione 10). Il PMO v2.0 (M+10) avrà confidence HIGH pre-gate G3.
>
