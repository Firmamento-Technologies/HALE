...
Il Volume 2 raccoglie gli **allegati tecnici** dello Studio di Fattibilità HALE/VTOL Firmamento Technologies. Ogni allegato fornisce evidenza quantitativa, tracciabile e verificabile a supporto della trattazione testuale del Volume 1. La struttura segue l'Allegato I.7 del Codice dei Contratti Pubblici (PFTE) e mantiene piena coerenza con la metodologia NASA SE Handbook adottata dal progetto.

I 13 allegati principali coprono l'intera catena di evidenze: tracciabilità requisiti (A.1), gestione del rischio (A.2), trade study architetturali (A.3), interfacce di sistema (A.4), piano V&V (A.5), CAD (A.6), modelli di calcolo (A.7), bilanci di massa (A.8), computo metrico (A.9), piano di manutenzione (A.10), safety case SORA (A.11), VIA preliminare (A.12) e documentazione fotografica (A.13).
...
| **A.3** | [`A3-Trade-Studies/`](A3-Trade-Studies/) | DOCFAP, Documento Fattibilità Alternative Progettuali | v1.0 |
...
| **A.5** | [`A5-VV-Plan/`](A5-VV-Plan/) | Verification & Validation Plan v1.0 (71 SyR, 7 sheet, €1.57M test) | v1.0 |
| **A.6** | [`A6-CAD/`](A6-CAD/) | Schemi/disegni CAD del concept (placeholder; CAD binari in `/cad/`) | v1.0 |
| **A.7** | [`A7-Link-Budget/`](A7-Link-Budget/) + [`energy-balance/`](energy-balance/) + [`financial-model/`](financial-model/) | Modelli di calcolo (3 modelli: link budget 14 scenari ITU-R, energy balance HALE 365gg, finanziario 10 sheet) | v1.0 |
| **A.8** | [`A8-Bilanci-Massa/`](A8-Bilanci-Massa/) | Bilanci di massa preliminari (6A + 6B) | v1.0 |
...
| **A.13** | [`A13-Documentazione-Fotografica/`](A13-Documentazione-Fotografica/) | Documentazione fotografica del contesto (placeholder, foto da acquisire) | v1.0 |
...
- **v1.0 M+3 bozza** (presente, copre la maggior parte degli allegati)
- **v1.5 M+6**: update post pre-application ENAC, workshop cooperative e vendor quotation reali
...
La confidence aggregata del Volume 2 si attesta su un livello **medium-low**: numerosi allegati richiedono validazione esterna. I DR ancora aperti (riferimento `riferimenti/audit-rigore-epistemico.md`) sono 6 voci che richiedono engagement esterno con Coopfond, ENAC, AGCOM, Garante, CIRA e Leonardo/TAS.

La simulazione di energy balance (allegato A.7 energy-balance) ha prodotto un **finding critico**: la perennial a 44°N non è fattibile con tecnologia baseline 2026-2028, dunque E5 Seasonal diventa mandatory plan A.

La capital intensity della Phase B 6B (allegato A.7 financial-model) si attesta a €5.5-13.5M, ovvero R&D Phase 0/A only (DR-014: benchmark $50M-1B per fase operativa).

Il pricing per la PA è stato ricalibrato post Cluster D: €60-90k base più €30-60k premium (vs €150k originale).
