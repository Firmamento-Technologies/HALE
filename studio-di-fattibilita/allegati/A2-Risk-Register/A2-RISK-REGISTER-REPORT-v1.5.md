...
**Totale rischi formalmente tracciati**: **119** (v1.5) — 116 v1.0 + 3 nuovi RSK-AMB ambientali (post A.12 VIA v2.0 §A.12.5)
...
*Nota: La colonna `Color` rappresenta lo **score baseline pre-mitigation** (per categorizzazione standard del registro). Per i 3 RSK-AMB: baseline RSK-AMB-001 = 12 YELLOW, RSK-AMB-002 = 9 YELLOW, RSK-AMB-003 = 8 YELLOW. Post-mitigation: tutti scendono (vedi §2.4). La conta "GREEN baseline 35" deriva da rischi GREEN gia da score baseline (es. RSK-REG-011); i 3 RSK-AMB sono YELLOW baseline (1) + 2 categorizzati nella tabella come post-mitigation GREEN per coerenza con §3 narrato.
...
Totali per fascia: RED = 17 (invariato vs v1.0), YELLOW = 68 (+1 RSK-AMB-001 a 3x4=12 + RSK-AMB-002 a 3x3=9 + RSK-AMB-003 a 2x4=8 -> conteggio reale +3 YELLOW), GREEN baseline = 34 invariato. Coerenza tabella §2.1: RED=17, YELLOW=68, GREEN(baseline)=34 — **totale 119**.
...
> **Nota di classificazione**: RSK-AMB-001 residual score = 6 (P=2, I=3) si trova al boundary GREEN/YELLOW (range GREEN 1-7). Per coerenza con criticita engagement Ente Parco e VIncA, **viene classificato YELLOW residual** (decisione risk-register-builder + ambientalista esterno, M+3) come segnale di sorveglianza attiva. Re-assessment M+9 post submission screening VIncA.
...
**Descrizione**: Disturbo avifauna nidificante in SIC IT1331402 Parco Antola — operazioni VTOL in stagione riproduttiva marzo-luglio possono indurre flushing aquila reale (*Aquila chrysaetos*, Allegato I Direttiva 2009/147/CE Uccelli) e gufo reale (*Bubo bubo*) con abbandono nido + perdita covata; attivazione automatica VIncA Livello II (DPR 357/1997 + L.R. Liguria 28/2009)  
...
**Residual P x I**: 2 x 3 = 6 (classificato **YELLOW residual** conservativo per criticita engagement Ente Parco + VIncA — boundary GREEN/YELLOW)  
...
**Confidence**: **medium** (limite operativo conservativo VTOL CW-30E classe rumore moderata; modello propagazione ISO 9613 affidabile; gestione esposti richiede engagement Comune Torriglia attivo)  
...
**Descrizione**: Impatto visivo paesaggistico VTOL in area vincolata D.Lgs. 42/2004 — sorvolo paesaggi tutelati Parco Antola puo configurare impatto visivo transitorio percepito dalla comunita + turisti, con potenziale opposizione associazioni ambientaliste (Italia Nostra, WWF, Legambiente) in fase istruttoria Regione Liguria DG Ambiente  
...
**Confidence**: **medium** (engagement associazioni ambientaliste e' efficace ma non certo; dipende da posizionamento storico Italia Nostra/WWF Liguria su progetti UAS — base rate Liguria recente moderatamente collaborativo)  
...
Programma di Early Warning Indicators dedicato alla nuova categoria Ambientale, articolato in monitoraggio trimestrale + integrazioni stagionali (bioacustica mar-mag) + log continuo (esposti, bird-strike).
...
**Cadenza review aggregato**: trimestrale (Q+1, Q+2, ...) con consolidamento in Risk Register quarterly review meeting (§8.2). Owner consolidato: **ambientalista esterno** (lead) + **ops** (co-lead operativo).
...
- **[NEW v1.5]** Engagement Ente Parco Antola + screening VIA + VIncA presentati entro M+9 (RSK-AMB-001/002/003) — milestone Cap. 5 §5.7bis + RTM REQ-NF-AMB-01

Scenario realistico (post Red Team M+3): 60-80% percorsi sono **Hold con piano** vs **Go pieno** al M+10/M+11. La nuova categoria Ambientale **non sposta il verdetto Go Condizionato 6A** (residual basso) ma aggiunge condizionalita engagement Ente Parco.

---

## 5. FMECA results - sintesi

Vedi fogli XLSX: `FMECA_Payload`, `FMECA_Avionica`, `FMECA_Propulsione`, **`FMECA_Ambientale` (nuovo v1.5 — preliminary)**.
...
> **Caveat metodologico**: la FMECA ambientale e' **preliminary** e basata su expert judgment risk-register-builder + ambientalista esterno + revisione preliminare letteratura disturbo avifauna UAV (Mulero-Pazmany 2017, Vas 2015). **Richiede engagement Ente Parco Antola + campagna bioacustica field Y1 (mar-mag) per consolidamento** (target re-baseline M+8-M+12). Severity, Occurrence, Detection sono stimati in scala MIL-STD-1629A 1-10. RPN = S × O × D.
...
**Mitigation obbligatoria** (RPN >= 100): 2 failure modes — flushing aquila reale (RPN 140) + bird-strike specie protetta (RPN 96, prossimo soglia). Entrambi mitigati da pacchetto M-AVI-01..07.

**Cross-link FTA**: il failure mode "bird-strike specie protetta" e' anche cut-set secondario nel top event **Loss of Vehicle in BVLOS** (§6.1) — contributo stimato ~5E-7/h (bird-strike rate generale UAS BVLOS bassa quota).

**Confidence FMECA Ambientale**: **medium** (preliminary, richiede engagement Ente Parco + bioacustica M+8-M+12 per validazione Occurrence; Detection da consolidare con campagna ARU Y1).
...
5. **[NEW v1.5]** Bird-strike specie protetta (~5E-7/h) - mitigato M-AVI-04 quota minima + M-AVI-07 abort
...
5. **[NEW v1.5]** Abort restrizione ambientale (avvistamento nidificazione, sforamento acustico, esposto cittadino) (~1-2%) - mitigato M-AVI-02/M-NOI-02
...
- **[NEW v1.5]** Categoria Ambientale: 0 RED, 1 YELLOW (RSK-AMB-001), 2 GREEN residual. Engagement Ente Parco e' condizionante ma non bloccante per gate G2/G3.
...
**Confidence aggregato categoria Ambientale**: **medium** (3 rischi su 3 con confidence individuale medium; mitigation richiede engagement Ente Parco — non ancora formalizzato; bioacustica field Y1 da pianificare; press monitoring associazioni ambientaliste continuo richiesto).

### 7.3 Caveat epistemico

Tutti i residual score sono **stime expert judgment** del risk-register-builder + safety engineer + **(nuovo v1.5)** ambientalista esterno, con confidence dichiarato per ogni rischio. La probabilita di mitigation effettiva al M+9-12 dipende da:
...
Re-assessment quarterly con re-scoring trimestrale. Per categoria Ambientale: **re-baseline M+8-M+12** post mappa nidi + bioacustica + DGR esito screening.
...
**Documenti generati**: Risk Register vN+1 (versioning) + EWI dashboard + escalation log + **[NEW v1.5] sezione dedicata Ambientale con sintesi EWI-AMB-01..10**
