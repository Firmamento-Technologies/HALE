...
> **Documenti predecessori:** `A1-RTM-REPORT.md` (v1.0 M+3 baseline estesa, 279 record) + `RTM-v1.0-full.csv` + `RTM-v1.0.xlsx`
> **Delta v1.5:** `RTM-v1.5-delta.csv` (7 record nuovi + 7 link di traceability — totale 14 righe delta)
...
La presente RTM v1.5 è un **incremento riproducibile** della baseline v1.0 (M+3 estesa, 279 record) costruito secondo la stessa metodologia del **NASA Systems Engineering Handbook Rev 2** (NASA/SP-2016-6105 Rev 2), in particolare §4.1 Stakeholder Expectations Definition, §4.2 Technical Requirements Definition, §4.3 Logical Decomposition (allocazione SsR) e §5.3-5.4 V&V Processes.
...
| Subsystem Requirements (SsR) | **84** | 81 | +3 (SsR-OPS-AMB-001 + SsR-AVI-AMB-001 + SsR-DAT-AMB-001) | ~80 (campione) |
...
| **TOTALE righe RTM** | **287** (record 285 + 2 link strutturali calcolati) | 279 | **+8 record + 7 link di traceability nuovi** | ~155 stimato |

> Nota di conteggio: il count "287" comprende i 285 record di requisito formali (StNeed/SyR/SsR/IR/NegR/VR) più 2 link strutturali calcolati che vengono normalmente rappresentati come riga distinta in tool RTM esterni (DOORS/Jama/Polarion); i 7 Link-001…007 del file `RTM-v1.5-delta.csv` sono **link di traceability esplicitati per chiarezza ispettiva** e non vanno computati come record indipendenti — sono già impliciti nei campi `Parent` dei 7 record nuovi. Il totale formale dichiarato in dashboard rimane quindi **285 record** (279 v1.0 + 6 nuovi requisiti + 0 modifiche destruttive).
...
> Nota architetturale sui nuovi sottosistemi OPS e DAT: in v1.0 l'analoga funzione era allocata implicitamente al sottosistema GS (Ground Segment), che però copre principalmente l'hardware/infrastruttura. L'introduzione di OPS (mission planning workflow + procedure operative) e DAT (data governance + retention + audit trail) come **sottosistemi logici autonomi** è coerente con NASA SE Handbook §4.3 (logical architecture) e con la prassi DO-178C (separazione concerns SW). La transizione completa di SsR-GS-006 (mission planning software) verso il sottosistema OPS e di SsR-GS-002/003 (data hosting + anonimizzazione) verso DAT è prevista nella v2.0 M+10/M+11.
...
I 21 requisiti più critici dello Studio (priorità H/Critical + falsifying observation con trigger osservabile esplicito) — la top-20 della v1.0 è interamente confermata; in v1.5 si aggiunge SyR-NF-AMB-001 in qualità di nuovo critico ambientale.
...
> **Nota critica v1.5**: Il nuovo requisito SyR-NF-AMB-001 entra nella **top-priority H** perché è **showstopper operativo Y1** — senza Convenzione operativa Ente Parco Antola firmata, la flotta VTOL non può operare sopra il Parco (e quindi non può operare a Pentema, sito pilota anchor). La falsifying observation è già parzialmente verificabile a M+9 (engagement workshop Ente Parco M+6 + outcome documentato entro M+9) prima della finestra critica gate G3 M+10/M+11.
...
Tutti i 29 StNeed hanno almeno un SyR figlio. Coverage 100%. StNeed-029 (Ente Parco Antola) ha SyR-NF-AMB-001 come unico figlio diretto; coerente con NASA SE §4.2 (1:1 mapping ammesso per requisiti di conformità normativa atomica).

### 5.2 Orphan SyR (0 al v1.5)

Tutti i 66 SyR hanno almeno un parent legittimo (StNeed o SyR padre o Boundary B1/B2 o capitolo del Cap. 5/7/8 di riferimento o bando Cooding). SyR-NF-AMB-001 ha parent StNeed-029 + Boundary B1 (cooperative + territorio).

### 5.3 Untestable SyR (0 al v1.5)

Tutti i 66 SyR hanno un metodo V&V definito (I/A/D/T o combinazione). Coverage 100%. SyR-NF-AMB-001 ha metodo composito **I+A+T** con VR-AMB-01 (Inspection audit annuale) + VR-AMB-02 (Test monitoring acustico).

### 5.4 Unallocated SyR (11 SyR decomponibili senza SsR — invariato v1.0)

Action item invariato rispetto a §5.4 v1.0. La nuova SyR-NF-AMB-001 è **già pienamente decomposta** in v1.5 con 3 SsR (OPS-AMB-001 + AVI-AMB-001 + DAT-AMB-001), quindi non aggiunge gap. Lista azione M+5-6 confermata: SyR-F-006, SyR-F-009, SyR-F-010, SyR-P-002, SyR-P-011, SyR-O-007, SyR-O-008, SyR-S-005, SyR-S-008, SyR-E-004, SyR-E-005, SyR-P-010 — 11 SyR da decomporre, ~10-15 ore engineer work per portare la coverage a 82-85% (target G2 80%).

### 5.5 NegR Audit Status (v1.5)

- **16/16 NegR Active** al M+3 (5 Critical + 7 High + 3 Medium-Low + **1 nuovo High NegR-AMB-016**).
...
Il gap evidenziato nella §5.6 della v1.0 (integrazione differita M+6) è **chiuso anticipatamente al M+3** con la presente v1.5:
...
| Workshop Ente Parco Antola M+6 per validazione | **OPEN — pianificato M+5-6** | engagement attivo via snai-funding-expert; output workshop integra confidence levels in v1.5.1 hotfix (se richiesto) |
...
> **Nota tracker**: 3 delle 25 OQ batch 2 M+3 risultano **chiuse o in progress** già a M+3 (OQ-019, OQ-020, OQ-021) grazie all'azione del subagent batch 2. Le rimanenti 22 OQ hanno target M+4-M+10 e sono trackate nel piano di azione M+6 (G2 entry) e M+10/M+11 (G3 entry). Aggiornamento status OQ avverrà alla v1.5.1 hotfix (atteso M+6) o alla v2.0 M+10 finale.
...
|---|---|---|---|---|
| v0.5 | M+3 (Cap. 3 baseline) | 17 StNeed + 42 SyR + ~80 SsR campione + 14 NegR | aerospace-SE | Storico |
...
> **Linkage A.12 VIA v2.0 + A.2 Risk Register v1.5 (refinement chain batch 2 M+3)**: La presente v1.5 è il **terzo deliverable di una triade integrata** prodotta nel batch 2 M+3:
> 1. **A.12 VIA Preliminare v2.0** identifica REQ-NF-AMB-01 in §A.12.6 mitigazioni + §A.12.3-A.12.4 evidenze biologiche/normative
> 2. **A.2 Risk Register v1.5** formalizza 3 RSK-AMB (001/002/003) in coerenza con mitigazioni A.12 v2.0
> 3. **A.1 RTM v1.5 (presente)** integra StNeed-029 + SyR-NF-AMB-001 + 3 SsR + 1 NegR + 2 VR in piena coerenza con A.12 v2.0 e A.2 v1.5
>
> Cross-reference Cap. 5 §5.7bis (atteso M+5-6) integrerà il quadro normativo ambientale dell'area pilota (L.R. Liguria 12/1995 + DPR 357/1997 + Direttiva Uccelli 2009/147/CE) e fornirà parent normativo al SyR-NF-AMB-001 per la coverage Cap. 5 → SyR.
...
Il file `RTM-v1.5-delta.csv` (~5 KB) contiene **solo i record nuovi del v1.5** rispetto al v1.0 baseline:
...
Per produrre il file `RTM-v1.5.xlsx` rigenerato al M+6, è necessario:
...
1. **Estendere `build_rtm.py`** (script Python esistente in `/studio-di-fattibilita/allegati/A1-RTM/build_rtm.py`) per:
   ...
   - Creare sheet nuovo `SyR_NF_ENV` con header standard 15 colonne + 1 riga (SyR-NF-AMB-001)
   - Aggiungere 3 righe al sheet `SsR_AVI` (SsR-AVI-AMB-001), e creare 2 nuovi sheet `SsR_OPS` (SsR-OPS-AMB-001) + `SsR_DAT` (SsR-DAT-AMB-001) — totale sheet xlsx: 14 → **16**
   - Aggiungere 1 riga a sheet `StNeeds` (StNeed-029) e 1 riga a sheet `NegR` (NegR-AMB-016) e 2 righe a sheet `VR` (VR-AMB-01 + VR-AMB-02)
   - Aggiornare sheet `Cover` con statistiche v1.5 (totali, breakdown SyR per famiglia + NF, breakdown SsR per sottosistema + OPS + DAT)
   ...
   - Mantenere color coding esistente + aggiungere palette specifica NF (es. verde scuro #548235 per famiglia NF Environmental Compliance)
...
4. **Commit Git** con messaggio "RTM v1.5 — integrazione A.12 VIA v2.0 (M+6)" + versionare il nuovo xlsx accanto al v1.0 (non sovrascrivere)

**Stima effort**: 2-3 ore engineer (estensione `build_rtm.py` + test + commit). Da pianificare in finestra M+5-6 prima del gate G2.

### 7.3 Sheet structure target v1.5 xlsx (M+6)

| # | Sheet | Contenuto v1.5 | Righe |
|---|---|---|---|
| 1 | Cover | Versione, data, boundary conditions, metodologia, statistiche v1.5, criteri G3 | ~70 (vs 65 v1.0) |
...
| 14 | Gap_Analysis | Riepilogo gap v1.5 (8 metriche) | ~10 + header |

### 7.4 Color coding v1.5 (esteso)

Schema invariato v1.0 + aggiunte:
...
- **Confidence overlay**: high=verde, medium=ambra (nuovo SyR-NF-AMB-001 + StNeed-029), low=rosso, boundary=blu

---

## 8. Note di chiusura del report v1.5

La RTM v1.5 chiude **anticipatamente a M+3 (batch 2)** il gap §5.6 della v1.0 (REQ-NF-AMB-01 + StNeed Ente Parco Antola integration) e raggiunge le seguenti metriche:
...
- **NegR Active: 16/16** + 0 violazioni — passing
...
1. **Workshop Ente Parco Antola M+5-6** (engagement attivo via snai-funding-expert + esg-sustainability-officer) per validazione formale StNeed-029 + SyR-NF-AMB-001 + acquisizione mappa nidi noti (input critico per SsR-OPS-AMB-001 buffer dinamici) + draft Convenzione operativa Y1 → output: v1.5.1 hotfix con confidence high.
2. **Decomposizione SsR M+5-6** per gli 11 SyR unallocated residui (azione invariata v1.0 §5.4) → coverage target 82-85% per G2.
3. **Pre-application meeting ENAC M+3-6** per validare SyR-F-002 (SAIL Pentema) + verifica compatibilità con SyR-NF-AMB-001 quota minima 200 m AGL su SIC (richiesta SORA potenzialmente più stringente nel buffer Parco) → falsifying observation per gate G2.
...
5. **Risk Register v1.5 ratifica board** (OQ-019 DONE batch 2 M+3) — verifica linkage RSK-AMB-001/002/003 → SyR-NF-AMB-001 + NegR-AMB-016 in coerenza Risk Register v1.5 (deliverable parallelo batch 2 M+3).
6. **Audit semestrale NegR M+9** prima del G3 → verifica 0 violazioni + waiver log up-to-date + **primo audit specifico NegR-AMB-016** (prima stagione critica mar-lug parzialmente coperta).
7. **Rigenerazione `RTM-v1.5.xlsx` M+6** (vedi §7.2bis) via `build_rtm.py` esteso — single source of truth Excel deve riflettere v1.5 prima del gate G2.
8. **V&V Plan dettagliato per VR Open → Planned** → 22 VR ancora in stato "Open" + 2 nuovi VR-AMB-01/02 (Planned, da pianificare con budget allocato ARPAL contractor + giornate Ente Parco).
...
- File `RTM-v1.5-delta.csv` (~5 KB) accanto a `RTM-v1.0-full.csv` (~88 KB) in `/studio-di-fattibilita/allegati/A1-RTM/`.
- File `RTM-v1.0.xlsx` **invariato** in attesa rigenerazione M+6 → `RTM-v1.5.xlsx`.
- Cross-reference Cap. 3 (RTM v0.5 baseline) + Cap. 5 §5.7bis (quadro normativo ambientale, atteso M+5-6) + Cap. 6 §6.4 (Safety Case + ambientale) + Cap. 9 §9.2 (gate G2 entry criteria deve includere SyR-NF-AMB-001 in coverage check).
...
- Confidence StNeed-029 + SyR-NF-AMB-001 + 3 SsR + NegR-AMB-016 + VR-AMB-01/02: tutti dichiarati **medium** in attesa di engagement formale Ente Parco Antola M+6 (workshop atteso M+5-6). Il refresh a high è pianificato in v1.5.1 hotfix M+6 post-workshop.
...
- Cross-validazione triangolata: A.12 VIA v2.0 §A.12.3-A.12.4 (evidenze biologiche fonti pubbliche secondarie, confidence medium-low) + A.2 Risk Register v1.5 §26-27-28 (3 RSK-AMB con score quantitativi e mitigazioni) + RTM v1.5 (presente, requisito formalizzato con metodo VAFC). La triade è auto-coerente ma **non validata da ente terzo** (RINA, DNV, ARPAL); validazione raccomandata per uso "investment-grade" prima del G5 (M+24).
- I numeri di buffer (500 m / 1000 m), quota minima (200 m AGL), e soglie acustiche (+3 dB(A) Leq,d) sono **derivati da best practice nazionali (vedi A.12 v2.0 §A.12.6) e da letteratura grigia ornitologica**, non da rilievi field dedicati al Parco Antola. La conferma operativa richiede mappa nidi noti Ente Parco + campagna fonometrica baseline M+10 (VR-AMB-02).
...
[^2]: **INCOSE Systems Engineering Handbook**, 5th Edition (2023) + INCOSE Guide for Writing Requirements (GtWR 2023).
...
[^7]: **Direttiva 2009/147/CE "Uccelli"** + Allegato I (specie di interesse comunitario). Specie rilevanti SIC IT1331402: aquila reale, gufo reale, pellegrino, biancone — fonte primaria per buffer NegR-AMB-016.

[^8]: **L.R. Liguria 12/1995** "Riordino delle Aree Protette" — istituzione Parco Naturale Regionale dell'Antola + competenze Ente Parco + sanzioni art.27 (fino €15.000 per violazione vincoli ambientali).

[^9]: **Decreto Min. Ambiente 16.03.1998** "Tecniche di rilevamento e di misurazione dell'inquinamento acustico" — strumentazione classe 1 IEC 61672 + indicatori Leq + zonizzazione aree protette Classe I. Linkato a VR-AMB-02.

[^10]: **Cap. 3 Requisiti e RTM** baseline v0.5: `studio-di-fattibilita/cap-03-requisiti-e-RTM.md`.

[^11]: **Cap. 5 Quadro Normativo** + 15 showstopper + atteso §5.7bis quadro normativo ambientale: `studio-di-fattibilita/cap-05-quadro-normativo.md`.

[^12]: **Cap. 6 Analisi tecnica** + Trade Studies: `studio-di-fattibilita/cap-06-analisi-tecnica.md` §6.3 + §6.4 Safety Case.

[^13]: **Cap. 9 Cronoprogramma e Gate**: `studio-di-fattibilita/cap-09-cronoprogramma-e-gate.md` §9.2 (Gate G0-G6 entry/exit criteria, G2 entry criteria deve includere SyR-NF-AMB-001 coverage check).
...
[^15]: **Allegato A.2 Risk Register v1.5**: `studio-di-fattibilita/allegati/A2-Risk-Register/A2-RISK-REGISTER-REPORT.md` §3bis + §26-27-28 (RSK-AMB-001/002/003) — refinement chain peer di RTM v1.5.
...
[^18]: **Skill `epistemic-rigor`**: `.claude/skills/epistemic-rigor/SKILL.md` — disciplina di falsifiability + confidence levels applicata.

[^19]: **Boundary conditions B1+B2** dichiarate in `CLAUDE.md` (project root) + `riferimenti/visione-10-anni.md` + `riferimenti/RESERVED-rischi-geopolitici.md`.
